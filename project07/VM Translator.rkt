...
(define (push-pointer) '("@~a" "D=M" "@SP" "A=M" "M=D" "@SP" "M=M+1"))
...
(define (pop-pointer) '("@SP" "A=M-1" "D=M" "@13" "M=D" "@~a" "D=A" "@THAT" "D=M+D" "@14" "M=D" "@13" "D=M" "@14" "A=M" "M=D" "@SP" "M=M-1"))
...
(define (return-vm) '("FRAME" "RET" "RET-RESULT" "RET-SP" "RET-THAT" "RET-THIS" "RET-ARG" "RET-LCL" "goto-ret ~a")) ;~a is a lable
...
;function
(define (function) '("(~a)" "@~a" "D=A" "(START)" "D=D-1" "@LCL" "A=M+D" "M=0" "A=D-1" "@START" "D;JNE" ))
...
(define (RET) '("@5" "D=A" "@FRAME" "D=M-D" "@RET" "M=D")) ;@RET is define in the SYM-TBL below to be @14
(define (RET-RESULT) '("@SP" "A=A-1" "D=M" "@ARG" "M=D")) ;@HOLD is defined in the sym table as @15
(define (RET-SP) '("@5" "D=A" "@FRAME" "D=M-D" "@SP" "M=D+1"))
(define (RET-THAT) '("@1" "D=A" "@FRAME" "A=M-D" "D=M" "@THAT" "M=D"))
(define (RET-THIS) '("@2" "D=A" "@FRAME" "A=M-D" "D=M" "@THAT" "M=D"))
...
(define (main)
  (define name
    (command-line #:args ([filename "input.vm"]) filename))


  (define lines (read-lines name))
  (define no-cmt (filter (λ (x) (and (not (void? x))
                                     (non-empty-string? x))) (map (λ (x) (eliminate-comments x)) lines)))
  (define translated (map (λ (x) (VM-to-Hack x)) no-cmt))

  (printf "Done")
 (define output-file-name (file-namechange name ".asm"))
 (write-file output-file-name (string-join translated "\n")))
...
(define (eliminate-comments a-str)
  (cond [(string-prefix? a-str "//") (void)]
        [(string-contains? a-str "//") (first (string-split a-str "//"))]
        [else a-str]))
...
(define (VM-to-Hack a-line)
  (if (or (string-contains? a-line "call")
...
(define (add-cmd-label a-lol)
  ...
  (map (λ (x) (cond [(void? x) x]
                    ...
                    [(and (string-contains? x "@")
                     ...
                     (define show2 (string-append x (number->string (hash-ref tbl (first (filter (λ (z) (string-contains? z (substring x 1 (- (string-length x) 4)))) (hash-keys tbl)))))))
...
(define (translate-function-cmd a-cmd)
  ...
  (cond [(string=? (first split) "call") (translate-call-hack (translate-call-vm (second split) (third split)))]
...
(define (translate-call-vm f n)
  (define result (map (λ (x) (if (string-contains? x "~a")
                                (format x f)
                                x)) (call-vm)))
...
(define (translate-call-hack a-intermediate)
...
(define (format-hash a-tbl a-seg i)
  ...
  (define next-i (cond [(string=? "pointer" a-seg) (+ i 3)]
                       [(string=? "temp" a-seg) (+ i 5)]
